...
import { generateWrongPDF } from '@/lib/pdf-generator'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get explanation mode from query parameter
    // If user has both languages active (explanationMode='both'), include both explanations
    // Otherwise default to showing both explanations in PDF for better learning
    const explanationModeParam = searchParams.get('explanationMode')
    const explanationMode = (explanationModeParam === 'both' || explanationModeParam === 'nl') 
      ? explanationModeParam as 'nl' | 'both'
      : 'both' // Default to 'both' to always show both Dutch and Arabic explanations
    ...
    const pdfBytes = await generateWrongPDF(review, locale, explanationMode)

    // Generate descriptive filename
    const modelTitle = attempt.model.titleNl || `Examen ${attempt.model.number}`
    const pdfType = 'Fouten en Uitleg'
